...
are outputting.
Debugging is very relavent in terms of what programmers have
to do on the daily. We debug, and we debug often.
There are many ways to debug, and it defers between situations.
But using javascripts console.log to see what our code is doing is the 
most basic (and probably the most used)
*/ 
...
in there and ask the terminal if it can fill out the folder and file for you
by using the "tab" button. (steps below) 
That way, you don't have to keep typing to whole path.

Go to your terminal and enter the command: clear
We just cleared the terminal for a fresh perspective.
Now, enter the command: ls
Look at the output.
If you havent guessed yet, this is your file structure at the current level of this lesson.
Now, lets ask node to finish some of the typing for us.
Note: (*tab = press the tab button)
In the terminal, type: node A*tab (no space after A)
...
You will see them a lot at the end of my code.
You will be happy to know that JavaScript code does not need these to run anymore.
But I still encourage you to write them.
Essentially, all professionals still use them even though they do not do anything
Programmer can argue all day why we should still use them,
